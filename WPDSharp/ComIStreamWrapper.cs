...
namespace WPDSharp
{
    internal class ComIStreamWrapper : Stream
    {
        private readonly IStream _istream;

        public ComIStreamWrapper(IStream istream)
        {
            _istream = istream;
        }
        ...
        {
            IntPtr newPos = new IntPtr();
            _istream.Seek(offset, (int)origin, newPos);

            _pos = newPos.ToInt64();

            return newPos.ToInt64();
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            _istream.Read(buffer, count, hBytesRead.AddrOfPinnedObject());
            _pos += (int) ((ulong) hBytesRead.Target);
            return (int)((ulong)hBytesRead.Target);
        }
